...
def un
num = Handle.size 
puts "Il y a #{num} dans cette array"
puts " "
end
#Quel est le handle le plus court de cette liste ?
def deux
puts "le plus petit est #{Handle.sort_by(&:length) [0]}"
puts " "
end
#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def trois
puts "Il y a #{Handle.count {|string| string.length == 6}} handles avec 5 caractères"
puts " "
end
#Combien commencent par une majuscule (première lettre juste après le @) ?	
def quatre
puts "Il y a #{Handle.count {|string| string[1] == string[1].upcase if string[1] != '_'}} handle commencant par une majuscule"
puts " "
end
...
def six
puts "#{Handle.sort_by{|str| str.length}}"
puts " "
end
#Quelle est la position dans l'array de la personne @epenser ?
def sept
puts "Bruce est le numéro #{Handle.index("@epenser")} de l'array"
puts " "
end
...
def menu
	...
	puts " "
	puts "Choisis ma fonction de 1 à 8"
    ...
    if menu >= 9
        puts "Entre 1 et 8, s'il te plait"
    end
    if menu <= 0
        puts "Entre 1 et 8, s'il te plait"
    end
